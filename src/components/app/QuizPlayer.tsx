      ...
      if (childId) {
        try {
          ...
          toast.error("Impossible d'enregistrer le quiz", {
            description: "Tes points seront ajoutés au prochain démarrage.",
          });
        }
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface-2 px-5">
        <div className="bg-surface border border-line rounded-modal p-8 max-w-md w-full text-center">
          ...
          <p className="text-sm text-fg-soft mb-8">{score} / {total} bonnes réponses</p>
          <div className="flex gap-3">
            <button onClick={() => router.push("/eleve")} className="btn btn-outline flex-1">Accueil</button>
            <button
              onClick={() => {
  ...
  return (
    <div className="min-h-screen bg-surface-2 flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-5 py-8 pb-32">
        <div className="text-xs font-semibold text-gold uppercase tracking-wider mb-3">Question {index + 1}</div>
        ...
        {picked !== null && (
          <div className="mt-6 bg-surface border-l-4 border-gold rounded-card p-5">
            <div className="text-xs font-semibold text-gold uppercase tracking-wider mb-2">Explication</div>
      ...
      <footer className="fixed bottom-0 inset-x-0 bg-surface border-t border-line p-4">
        <div className="max-w-2xl mx-auto flex justify-between gap-3">
          <div className="flex gap-2">
            <button className="w-11 h-11 rounded-full bg-cream dark:bg-surface-3 text-fg" title="Indice" aria-label="Indice">💡</button>
            <button className="w-11 h-11 rounded-full bg-cream dark:bg-surface-3 text-fg" title="Audio" aria-label="Audio">🔊</button>
          </div>
          <button
            onClick={onNext}
            disabled={picked === null}
